...
func plan(root string, schema Schema, options Options) ([]outputFile, error) {
	...
	componentsYAML, err := renderComponents(schema.Components)
	if err != nil {
		return nil, err
	}
	data := templateData{
		...
		Routes: routes(schema.Permissions), Modules: registryValue.Modules, ComponentsYAML: componentsYAML,
	}
	specs := []struct {
		template string
		path     string
...
func renderComponents(components map[string]map[string]any) (string, error) {
	if len(components) == 0 {
		return "", nil
	}
	content, err := yaml.Marshal(components)
	if err != nil {
		return "", fmt.Errorf("encode OpenAPI components: %w", err)
	}
